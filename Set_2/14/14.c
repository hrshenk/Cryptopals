...
void main()
{
    unsigned int length,block_size, i, j, k=0, block_fill, input_length, decrypt_success, target_flag =0;
    ...
    //determine how many characters required to align the prepended text to the block size.    
    while(mode == -1)
    {
        user_input[i] = 'a';
        length = oracle(user_input);
    ...
    block_fill = i%block_size;
    printf("prepend size is 16 * %d\n", mode);
    printf("block_fill is %d\n", block_fill);
    //we will pad out the prepended text to be aligned with a block using 'f' chars
    for(i=0; i<block_fill; i++)
    {
        user_input[i] = 'f';
    }
    
    //via repeated calls to the oracle the below chunk will decrypt the target ciphertext
    while(k<Input_Size-block_fill)
    {
        //clear the user_input array except for our 'f' chars
        for(i=0; i < Input_Size-block_fill; i++)
        {
            user_input[i+block_fill] = 0;
        }
        //construct user_input so plaintext in oracle will hold the next unknown byte
        for(i=0; i<(block_size-1)-(k%block_size); i++)
        {
             user_input[i+block_fill] = 'a';
        }
        ...
        for(i = ((block_size - 1) - (k % block_size)); i<Input_Size-block_fill; i++ )
        {
            user_input[i + block_fill] = array[i-((block_size - 1) - k%block_size)];
        ...
        for(i= 1 ; i< 255; i++) //we can skip non-printable chars since we're assuming printable chars. 
        {
            ...
            for(j=0; j< (input_length + (16*mode))-block_fill; j++)
            {
            ...
            if(j == (input_length + (16*mode))-block_fill)
            {
...
//The oracle function has been modified to prepend characters to the submitted input in addition to
//appending the contents of the file.  This forces the 'attacker' to do a little more work in order
//to decrypt the contents of the file.
unsigned int oracle(unsigned char *user_input)
{
    
    //to simulate a server prepending a fixed length string to our input we'll generate
    //a random number, and prepend that many random non-zero chars to the user input
    //before encrypting.
    static unsigned char prepend_flag = 0, prepend_size, prepend[256];
    ...
    srand(time(NULL));
    //if prepend_flag is 0, then we'll generate a random number of nonzero bytes
    //for the oracle to prepend and store them in prepend[]
    if(!prepend_flag)
    {
        prepend_size = rand();
        printf("prepend_size is : %d\n", prepend_size);
        prepend_flag = 1;
        while(i < prepend_size)
        {
            if((prepend[i] = rand()))
            {
                //printf("char:  %02X, i = %d\n", prepend[i], i);
    ...
    */
    ...
    if((file_size + prepend_size) >= (compute_max_int() - Input_Size)) //we could allow file_size to be bigger since conversion
    ...
    base64 = (unsigned char*)malloc(file_size + 1);
    if(base64 == NULL)
    {
   ...
   //prepend chars
   strncpy(buf, prepend, prepend_size);
    //transfer user_input to buf
    for(i=0; i<Input_Size - 1; i++)
    {
        buf[i+prepend_size] = user_input[i];
        if(user_input[i]==0)
        {
            break;
        }
    }
    buf[i+prepend_size] = 0;
    ...
    plaintext_length += user_input_size;  //determine length of prepend||userinput||decoded file input for encryption
